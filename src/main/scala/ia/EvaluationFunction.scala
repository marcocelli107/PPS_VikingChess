...
object EvaluationFunction {
  ...
  private def wrongBarricadeType(firstCoordinate: BoardCell, secondCoordinate: BoardCell): Piece.Value =
  ...
  private def isRowOrColumnOwner(cellsSeq: Seq[BoardCell]): Piece.Value = {
  ...
  private def checkOrthogonalEmptyCells(piece: Piece.Value, adjOrthogonalCells: Map[OrthogonalDirection, Option[BoardCell]]): Boolean = {
    var returnValue = false
