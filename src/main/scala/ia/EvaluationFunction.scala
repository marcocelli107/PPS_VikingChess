...
object EvaluationFunctionImpl {
  ...
  def apply(boardSize: Int): EvaluationFunction = EvaluationFunctionImpl(boardSize: Int)

  case class EvaluationFunctionImpl( boardSize: Int ) extends EvaluationFunction {
    ...
    * */
    ...
    def scoreKingNearCorners(kingCoord: Coordinate): Int = if (isItDistantFromCornerOf(kingCoord,1)) 90 else 0
